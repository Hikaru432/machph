                ...
                if ($companyid) {
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) { ?>
                            <tr>
                                <td>
                                    <a href="view_user_car_details.php?user_id=<?php echo $row['user_id']; ?>&car_id=<?php echo $row['car_id']; ?>" class="text-decoration-none fw-bold text-primary">
                                        <?php echo $row['name']; ?>
                                    </a>
                                </td>
                                ...
                                <td>
                                        <?php
                                        // Check if the car has been approved in the detailsapprove table
                                        $detailsapprove_query = "SELECT verify FROM detailsapprove WHERE user_id = '{$row['user_id']}' AND car_id = '{$row['car_id']}' AND companyid = '$companyid'";
                                        $detailsapprove_result = mysqli_query($conn, $detailsapprove_query);
                                        
                                        // Default to 'Need verify' message if the query returns no result or it's not 'Approved'
                                        ...
                                        if ($detailsapprove_result && mysqli_num_rows($detailsapprove_result) > 0) {
                                            ...
                                            if ($detailsapprove_row['verify'] === 'Approved') {
                                                $verify_status = 'Approved'; // If approved, proceed to mechanic assignment or display assigned mechanic
                                            }
                                        }

                                        // Check if the car already has an assigned mechanic in the assignments table
                                        ...
                                        if ($verify_status === 'Approved' && $assignment_result && mysqli_num_rows($assignment_result) > 0) {
                                            // If a mechanic is already assigned, display their details
                                            ...
                                            // If no mechanic is assigned yet, display the dropdown for assigning
                                            ?>
                                            ...
                                            // If not approved, display the 'Need verify' message
                                ...
                                <td>
                                    <?php if ($row['status'] === '0') { ?>
                                        <button type="button" class="btn btn-danger btn-sm btn-delete" data-service-id="<?php echo $row['car_id']; ?>" aria-label="Delete service">
                        ...
                        echo "<tr><td colspan='8' class='text-center'>No records found for the company.</td></tr>";
                    }
                } else {
                    echo "<tr><td colspan='8' class='text-center'>Company ID not found in session.</td></tr>";
                }
                ?>
            </tbody>
        </table>  
